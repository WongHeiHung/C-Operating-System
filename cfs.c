...
void execute_cfs_scheduling()
{
    ...
    for(int step = 0;;step++){
        ...
        if(step != 0){
            int chosen_process = choose_process();
            int runtime = process[chosen_process].time_slice;
            if(runtime > process[chosen_process].remain_time)
                runtime = process[chosen_process].remain_time;

            process[chosen_process].vruntime += (double) DEFAULT_WEIGHT / process[chosen_process].weight * runtime;
            process[chosen_process].remain_time -= runtime;

            if(chart[num_chart_item - 1].pid == chosen_process && num_chart_item != 0){
                chart[num_chart_item - 1].duration += runtime;
            }
            else{
                chart[num_chart_item].pid = chosen_process;
...
int choose_process(){
    ...
    for (int i = 0; i < num_process; i++){
        if(process[i].vruntime < min_value && process[i].remain_time != 0){
            min_index = i;
            min_value = process[i].vruntime;
        }
    }
    return min_index;
}
